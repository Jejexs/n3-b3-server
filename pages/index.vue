<template>
    <div class="bg-gradient-to-tr from-slate-100 to-slate-300 min-h-screen">
        <div class="m-auto max-w-screen-sm py-12">
            <h3 class="text-3xl font-semibold mb-4">Generate USERS</h3>
            <div class="my-4">
            ...
            <div v-if="loading" class="my-8" name="svg-spinners:blocks-wave"></div>
            <div v-if="error" class="my-8 text-red-600">{{ error }}</div>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-4">
                <div v-for="(person, index) in data.persons.slice(0, count)" :key="index"
...
const { data, loading, error, refresh, pending } = useFetch('/api/person', {
    query: {
        count
    },
    headers: {
